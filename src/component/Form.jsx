...
const Form = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      //   );
      // });
      const allSubmitted = responses.map((responseArray, index) => {
  const endpoint = apiEndpoints[index];

  // Skip isSubmitted check for basicDetails and advancedDetails
  const isBasicOrAdvanced =
    endpoint.includes("/basicDetails") || endpoint.includes("/advancedDetails");
console.log(isBasicOrAdvanced,"isBasicAdvanced")
  return (
    responseArray.length > 0 &&
    (isBasicOrAdvanced || responseArray.every((data) => data?.isSubmitted === true))
  );
});
