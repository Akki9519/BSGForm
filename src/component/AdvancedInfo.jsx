...
//     try {
//       const storedIdString = localStorage.getItem("_id");
//       console.log(storedIdString,"storedIdString")
...
const BasicForm = () => {
  ...
  const [selectedSubWings, setSelectedSubWings] = useState([]);
  const [courseDisable, setCourseDisable] = useState([]);
  const [isSubmitted, setIsSubmitted] = useState([]); // New state to track submission
          ...
          <div className="mt-8 space-y-6">
            {courses.map((course) => (
              <div key={course._id} className="p-4 border border-gray-300 rounded mb-4">
                {/* <h3 className="font-bold text-lg">Basic Course Details</h3> */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-20 gap-y-5">
                  <div>
                  ...
                  <div>
                    <strong>From Date:</strong> {course.fromDate}
                  </div>
                  <div>
                    <strong>To Date:</strong> {course.toDate}
                  </div>
                  <div>
                  ...
                  <div>
                    <strong>Certificate Date:</strong> {course.certificateDate}
                  </div>
                </div>
              </div>
            ))}
