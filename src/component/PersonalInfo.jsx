...
const PersonalInformation = () => {
  ...
  const handleFileChange = (file, setFile) => {
    if (file.size >  500 * 1024) { // Check if file size exceeds 200 KB
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
     ...
     if (uploadPhoto && uploadPhoto.size > 500 * 1024) {
      toast.error("Upload Photo size must be less than 500 KB.");
      setLoading(false);
      return;
    }
    if (uploadWarrant && uploadWarrant.size > 500 * 1024) {
      toast.error("Upload Warrant size must be less than 500 KB.");
      setLoading(false);
      return;
    }

    // Check if the uploaded highest qualification exceeds the size limit
    if (highestqualification && highestqualification.size > 500 * 1024) {
      toast.error("Upload Highest Qualification size must be less than 500 KB.");
      setLoading(false);
      return;
    }
